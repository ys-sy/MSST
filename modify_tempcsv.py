...
train_temp_path = sys.argv[1]
annotations_csv_path = sys.argv[2]
train_temp = []
    ...
    for index2 in range(len(train_temp)):
        ...
        if videoName == data[0]:
            if index2 == index:
                ...
                # 并且查查ava_train_temp[index]后面4个的坐标是否与ava_train_temp[index]一致
...
with open(annotations_csv_path,"w") as csvfile: 
